    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-surface-card border border-border-card rounded-card p-4 col-span-2 sm:col-span-1">
          ...
          <p className="text-white text-xl font-bold font-heading">{transactions.length}</p>
        </div>
      </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((tx) => (
              <div key={tx.id} className="flex items-center gap-4 px-5 py-4 hover:bg-primary-navy/50 transition-colors">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${tx.type === "PAYOUT" ? "bg-success/10" : "bg-accent-gold/10"}`}>
                  {tx.type === "PAYOUT"
